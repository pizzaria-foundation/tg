...
`C:\Data\logs_telegram.txt` on the phone and streams live to `epoc db serve` when the bridge is
up. `DEBUG=0` removes those call sites entirely, format strings included, so leaving
